...
def extract_modelo_after_linea(data, linea_index):
    ...
    for page in data['analyzeResult']['readResults']:
        lines = page['lines']
        for j, next_line in enumerate(lines[linea_index + 1: linea_index + 4]):
...
def extract_color_after_modelo(data, modelo_index):
    ...
    for page in data['analyzeResult']['readResults']:
        lines = page['lines']
        for j, next_line in enumerate(lines[modelo_index + 1: modelo_index + 9]):
            next_text = next_line['text'].strip()
            if re.search(r'\d\.\d{3}', next_text):
...
def extract_clase_vehiculo(data):
    clases_vehiculo = ["CAMIONETA", "CAMION", 'MICROBUS']
    for page in data['analyzeResult']['readResults']:
        lines = page['lines']
        for line in lines:
...
def es_nombre_valido(texto):
    ...
    return not any(char.isdigit() for char in texto) and "IDENTIFICACION" not in normalize_text(texto).upper()
...
def extract_nombre_propietario(data):
    for page in data['analyzeResult']['readResults']:
        ...
        for i, line in enumerate(lines):
            ...
            # Buscar la frase clave "PROPIETARIO APELLIDO(S) Y NOMBRE(S)"
            # Comparar correctamente ambas posibles cadenas dentro del if
            if "PROPIETARIO: APELLIDO(S) Y NOMBRE(S)" in normalized_text or "PROPIETARIO APELLIDO(S) Y NOMBRE(S)" in normalized_text:
                print(normalized_text)
...
# Función para extraer la identificación del propietario basándose en las condiciones dadas
def extract_identificacion_propietario(data):
    for page in data['analyzeResult']['readResults']:
        ...
        for i, line in enumerate(lines):
            ...
            # Buscar la frase clave "PROPIETARIO APELLIDO(S) Y NOMBRE(S)"
            if "PROPIETARIO: APELLIDO(S) Y NOMBRE(S)" in normalized_text or "PROPIETARIO APELLIDO(S) Y NOMBRE(S)" in normalized_text:
                # Buscar la identificación en las siguientes 6 líneas
                for j in range(1, 4):
                    if i + j < len(lines):
                        next_text = lines[i + j]['text'].strip()
...
with open('./data8.json', 'r', encoding='utf-8') as file:
    data = json.load(file)
...
# Extraer el nombre del propietario del vehículo
nombre_propietario = extract_nombre_propietario(data)

# Extraer la identificación del propietario del vehículo
identificacion_propietario = extract_identificacion_propietario(data)
...
print(f"Color: {color}")
print(f"Clase del Vehículo: {clase_vehiculo}")
print(f"Combustible: {combustible}")
print(f"Tipo de Carrocería: {tipo_carroceria}")
print(f"Número de Motor: {numero_motor}")
print(f"VIN: {vin}")
print(f"Número de Serie: {numero_serie}")
print(f"Número de Chasis: {numero_chasis}")
print(f"Nombre del Propietario: {nombre_propietario}")
print(f"Identificación del Propietario: {identificacion_propietario}")
